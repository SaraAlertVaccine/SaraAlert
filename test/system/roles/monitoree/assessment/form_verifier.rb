...
class AssessmentFormVerifier < ApplicationSystemTestCase
  ...
  def verify_assessment(patient, assessment, submission_token)
    ...
    assert saved_assessment.present?, @@system_test_utils.get_err_msg('Recipient assessment', 'assessment', 'existent')
    ...
    assessment['symptoms'].each do |symptom|
      ...
      case symptom['type']
      when 'BoolSymptom'
        err_msg = @@system_test_utils.get_err_msg('Recipient assessment', symptom['label'], symptom['bool_value'])
        assert_equal symptom['bool_value'], saved_symptom['bool_value'], err_msg
      when 'FloatSymptom'
        err_msg = @@system_test_utils.get_err_msg('Recipient assessment', symptom['label'], symptom['float_value'])
        assert_equal symptom['float_value'], saved_symptom['float_value'], err_msg
      when 'IntegerSymptom'
        err_msg = @@system_test_utils.get_err_msg('Recipient assessment', symptom['label'], symptom['int_value'])
