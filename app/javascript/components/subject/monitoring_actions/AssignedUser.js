...
class AssignedUser extends React.Component {
  ...
  handleAssignedUserChange = assigned_user => {
    this.setState({ assigned_user }, this.toggleAssignedUserModal);
  };
  ...
  toggleAssignedUserModal = () => {
    this.setState(state => {
      return {
        showAssignedUserModal: !state.showAssignedUserModal,
        assigned_user: state.showAssignedUserModal ? this.props.patient.assigned_user : state.assigned_user,
        apply_to_household: false,
        reasoning: '',
      };
    });
      ...
      <React.Fragment>
        <div className="disabled">
          ...
          <Form.Group className="d-flex mb-0">
            <AssignedUserInput
              workflow={this.props.patient.isolation ? 'exposure' : 'isolation'}
        ...
        <Modal size="lg" show={this.state.showAssignedUserModal} centered onHide={this.toggleAssignedUserModal}>
          <Modal.Header>
          ...
          <Modal.Footer>
            <Button variant="secondary btn-square" onClick={this.toggleAssignedUserModal}>
              Cancel
            </Button>
